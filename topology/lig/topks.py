#!/panfs/panfs.ihep.ac.cn/home/data/lig/local/bin/python3.1
#!/usr/bin/python
...
##
def process(event):
    global fdict, flist, fnumb, fchan, nevt, itop, itot, lined, ichan,cutnum,cutlines,fanythingDict,fanythingList,fanythingChan,fanythingNum,totalEvents
    number = []
    pdgid  = []
    mother = []
    allparticle = []
    ...
    while (i<len(number)):
        ...
        if number.count(mother[i]) >= 1 : # be a daugther of anyone
           if (mother.count(number[i])==0 or pdgid[i] == '111' or pdgid[i] == '310'): # has no daugther/final state or is pi0
              if (pdgid[int(mother[i])] != '111' and pdgid[int(mother[i])] != '310' and abs(int(pdgid[int(mother[i])])) != 211 and abs(int(pdgid[int(mother[i])])) != 321 ): # mother is not pi0
#           if (mother.count(number[i])==0 or pdgid[i] == '111' ): # has no daugther/final state or is pi0
#              if (pdgid[int(mother[i])] != '111' and abs(int(pdgid[int(mother[i])])) != 211 and abs(int(pdgid[int(mother[i])])) != 321 ): # mother is not pi0
#           if (mother.count(number[i])==0 or pdgid[i] == '111' ): # has no daugther/final state or is pi0
#              if (pdgid[int(mother[i])] != '111' ): # mother is not pi0
        ...
        if mother.count(number[i]) > 1 :  # has daughers/be a mother
           ...
           j=0
           while (j<len(number)):
              if (mother[j] == number[i] and abs(int(pdgid[int(number[i])])) != 321 and abs(int(pdgid[int(number[i])])) != 211 and abs(int(pdgid[int(number[i])])) != 111 and abs(int(pdgid[int(number[i])])) != 22 ):
...
#    print linel
    ...
    #print lined
    #print allparticle
    for single in fanythingList : 
	  if  single != '311' and allparticle.count(single) >= 1 :
	    fanythingNum[single]+=nevt
	  if  single == '311' and (allparticle.count('311') >= 1 or allparticle.count('-311')):
	    fanythingNum[single]+=nevt
#    if allparticle.count('22') >= 1 : 
#      totalEvents+=nevt
#      print totalEvents
    local="%3i&$%s$&%5i&%5i\\\\\n"%(ichan,lined[:-2], itop, nevt)
#	for single in fanythingList :
#		if single != '310' and single in line0 :
#       		fanythingNum[single]=fanythingNum[single] + nevt
#       		fanythingChan[single]=fanythingChan[single] + 1 
#       		if fanythingChan[single] < cutlines:
#       		   fanythingDict[single]=fanythingDict[single]+local
#		if single == '310' and ('310' in line0 or '-310' in line0 ):
#       		fanythingNum[single]=fanythingNum[single] + nevt
#       		fanythingChan[single]=fanythingChan[single] + 1 
#       		if fanythingChan[single] < cutlines:
#       		   fanythingDict[single]=fanythingDict[single]+local
#		print fanythingDict[single]	
    pline=''
    for ii in line0:
       pline+=partex[ii]
...
fanythingList = ['-11','-13','-321','311','321','-323','323','-313','313','221','331','333']
#		"-13" : '',
#	   "-321" : '',
#	    "310" : '',
#	    "321" : '',
...
if line!='':
   if lined !='':
      f.write(lined)
f.writelines(tab_tail)
for single in fanythingNum :
#  print ("%10s	%1.7s+/-	%1.8s"%(partex[str(single)],fanythingNum[single]/5210694.,((sqrt(fanythingNum[single]))) ))#5210694  #4086777
  print ("%10s	%1.7s+/-	%1.8s"%(partex[str(single)],fanythingNum[single]/4086777.,((sqrt(fanythingNum[single]))) ))#5210694  #4086777
#print totalEvents
print ("%5i %s"%(imode, file1))
h.write('\\begin{landscape} '+'\include{'+file1+'} '+'\\end{landscape} \\clearpage  \n')
f.close()
